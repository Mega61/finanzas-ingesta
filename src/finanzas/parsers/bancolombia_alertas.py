...
IGNORAR = [
    ...
    (
        'factura_disponible',
        r'(su factura inscrita.*?se vence|La factura que inscribiste.*?esta lista)',
    ),
    (
        'clave_dinamica',
    ...
    (
        'aviso_extracto',
        r'(disponible tu extracto|tu extracto (mensual|del mes|para el)'
...
@_familia(
    'compra_tcred',
    r'Compraste\s+(COP|USD)\s*([\d.,]+)\s+en\s+(.+?)\s*con tu T\.?\s?Cred\s*\*+\s?(\d{3,4})\s*,?\s*el\s+([\d/]+)\s+a las\s+([\d:]+)',
)
def _f1(m, msg):
...
@_familia(
    'compra_asociada',
    r'Compraste\s+(COP|USD)\s*([\d.,]+)\s+en\s+(.+?)\s*,\s*el\s+([\d/:]+)\s+a las\s+([\d/:]+)\.?\s*Esta compra esta asociada a T\.?\s?Cred\s*\*+\s?(\d{3,4})',
)
def _f2(m, msg):
...
@_familia(
    'pago_qr',
    r'pagaste\s+\$\s*([\d.,]+)\s+por codigo QR desde tu cuenta\s*\*+\s?(\d{3,4})\s+a la llave\s+(\S+)\s+el\s+([\d/]+)\s+a las\s+([\d:]+)',
)
def _f3(m, msg):
    return Evento(
        'pago_qr',
        parse_fecha(m.group(4)),
        m.group(5),
        'COP',
        -parse_monto(m.group(1)),
        m.group(2),
        'cuenta',
        m.group(3),
        f'PAGO QR llave {m.group(3)}',
        'pago_qr',
    )
...
@_familia(
    'transf_llave',
    r'transferiste\s+\$\s*([\d.,]+)\s+a la llave\s+(\S+)\s+desde tu cuenta\s*\*+\s?(\d{3,4})\s+a\s+(.+?)\s+el\s+([\d/]+)\s+a las\s+([\d:]+)',
)
def _f4(m, msg):
    dest = m.group(4).strip()
    return Evento(
        'transferencia_salida',
        parse_fecha(m.group(5)),
        m.group(6),
        'COP',
        -parse_monto(m.group(1)),
        m.group(3),
...
@_familia(
    'transf_cuenta',
    r'Transferiste\s+\$\s*([\d.,]+)\s+(?:por QR\s+)?desde tu cuenta\s*\*?\s?(\d{3,4})\s+a la cuenta\s*\*?\s?(\d+)\s*,?\s*el\s+([\d/]+)(?:\s+a las)?\s+([\d:]+)',
)
def _f5(m, msg):
    return Evento(
        'transferencia_salida',
...
@_familia(
    'transf_entrada',
    r'recibiste una transferencia de\s+(.+?)\s+por\s+\$\s*([\d.,]+)\s+en tu cuenta\s*\*+\s?(\d{3,4}).*?el\s+([\d/]+)\s+a las\s+([\d:]+)',
)
def _f6(m, msg):
    org = m.group(1).strip()
    return Evento(
...
@_familia(
    'pago_producto',
    r'Pagaste\s+\$\s*([\d.,]+)\s+a\s+(.+?)\s+desde tu producto\s*\*?\s?(\d{3,4})\s+el\s+([\d/]+)\s+([\d:]+)',
)
def _f7(m, msg):
    ben = m.group(2).strip()
    return Evento(
...
@_familia(
    'ingreso_nomina',
    r'Recibiste un pago\s+(?:de\s+)?(Nomina|PROVEEDOR)?\s*(?:de\s+)?(.+?)\s+por\s+\$\s*([\d.,]+)\s+en tu cuenta de Ahorros\s+el\s+([\d/]+)\s+a las\s+([\d:]+)',
)
def _f8(m, msg):
...
@_familia(
    'ingreso_cuenta',
    r'Recibiste un pago por\s+\$\s*([\d.,]+)\s+de\s+(.+?)\s+a tu cuenta\s+(\S+?)\s*,\s*el\s+([\d/:]+)\s+a las\s+([\d/:]+)',
)
def _f9(m, msg):
    f, h = _ordenar_fecha_hora(m.group(4), m.group(5))
    org = m.group(2).strip()
...
@_familia(
    'debito_tarjeta',
    r'Debitamos de tu cuenta/bolsillo\s+\$\s*([\d.,]+)\s+para abonar a la deuda.*?tarjeta de credito\s+(\S+)\s*\*+\s?(\d{3,4})',
)
def _f10(m, msg):
    ...
    ev = Evento(
        ...
        f'ABONO DEUDA {m.group(2)} {m.group(3)}',
        'debito_tarjeta',
    )
...
@_familia(
    'boton_bancolombia',
    r'Transferiste\s+\$\s*([\d.,]+)\s+por Boton Bancolombia a\s+(.+?)\s+desde producto\s*\*+\s?(\d{3,4})\.?\s*([\d/]+)\s+([\d:]+)',
)
def _f11(m, msg):
    ben = m.group(2).strip()
    return Evento(
...
@_familia(
    'transf_entrada_simple',
    r'Recibiste una transferencia por\s+\$\s*([\d.,]+)\s+de\s+(.+?)\s+en tu cuenta\s*\*+\s?(\d{3,4})\s*,?\s*el\s+([\d/]+)\s+a las\s+([\d:]+)',
)
def _f12(m, msg):
    org = m.group(2).strip()
    return Evento(
...
@_familia(
    'avance',
    r'Hiciste un avance de\s+\$\s*([\d.,]+)\s+en tu\s+(.+?)\s+el\s+([\d/:]+)\s+([\d/:]+)\s+desde tu T\.?\s?Credito\s*\*+\s?(\d{3,4})\s+a la cuenta\s*\*+\s?(\d{3,4})',
)
def _f13(m, msg):
