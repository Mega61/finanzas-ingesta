...
from finanzas.aplicacion import clasificador
from finanzas.aplicacion.publicador import ETIQUETA
...
def conciliar_extracto(cx, ext, dry_run=True, tasa_respaldo=None):
    ...
    for p, m, clase in pares:
        if clase == 'igual':
            ...
            registro.aviso(
                f'    corrige   {p["fecha"]} {p["valor"]:>13,.2f} -> '
            ...
            registro.aviso(
                f'    ambiguo   {p["fecha"]} {p["valor"]:>13,.2f} '
    ...
    for p in sin:
        ...
        registro.aviso(
            f'    FANTASMA  {p["fecha"]} {p["valor"]:>13,.2f} '
    ...
    try:
        if p['firefly_id']:
            firefly.cambiar_monto(
                ...
                m.valor,
                nota_extra=(
                    f'Monto corregido con el extracto {ext.archivo}: '
...
def _marcar_fantasma(cx, p, ext):
    ...
    db.pendiente_actualizar(
        cx, p['id'], estado='publicado', pregunta='existencia', visto_en=None
    )
    db.bitacora(
        cx,
        'sospecha_fantasma',
...
def correr(cx, carpeta=None, dry_run=True, solo=None):
    ...
    if not clave:
        registro.aviso('falta EXTRACTO_CLAVE (la cedula) para abrir los PDF')
        return {}
    carpeta = carpeta or config.ruta_personal('Extractos Bancolombia', '_pdf')
    if not os.path.isdir(carpeta):
        registro.aviso(f'no existe la carpeta de extractos: {carpeta}')
        return {}
    ...
    registro.aviso(
        f'{len(exts)} extractos leidos' + ('   [SECO, no escribe]' if dry_run else '')
    )
    ...
    if tasa_global:
        registro.aviso(f'tasa COP/USD de respaldo: {tasa_global:,.0f}')
    ...
    for e in sorted(exts, key=lambda x: (x.periodo_archivo, x.instrumento)):
        ...
        if c:
            registro.aviso(f'  {e.archivo}  ({e.desde} -> {e.hasta})')
            registro.aviso(f'    {", ".join(f"{k}={v}" for k, v in c.items())}')
            for k, v in c.items():
