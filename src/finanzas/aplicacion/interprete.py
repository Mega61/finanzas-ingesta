...
from finanzas.aplicacion import clasificador, presupuestos, taxonomia
...
SINONIMOS = {
    ...
    'TRANSPORTE MOTO': ['GASOLINA', 'TANQUEAR', 'TANQUEE', 'COMBUSTIBLE', 'TANQUEO'],
    'GIMNASIO': ['GYM', 'GIMNASIO', 'ENTRENO', 'MENSUALIDAD'],
    'CUIDADO PERSONAL': ['BARBERIA', 'PELUQUERIA', 'UNAS', 'MANICURE', 'PEDICURE'],
...
def _buscar_categoria(texto, categorias):
    ...
    for c in categorias:
        comunes = [
            x for x in (tk & _tokens(c)) if len(x) >= 5 and x not in PALABRAS_VAGAS
        ]
        if comunes:
...
def _buscar_comercio(texto, comercios, excluir=None):
    ...
    for m in comercios:
        ...
        comunes = [
            t
            for t in (tk & mt)
            if len(t) >= 5 and t not in PALABRAS_VAGAS and t not in vetadas
        ]
...
def similares(cx, usuario_id, pendiente, texto, limite=12):
    ...
    for r in filas:
        ...
        if comunes:
            puntuadas.append(
                (len(comunes), r['patron'], r['categoria'], r['presupuesto'])
            )
...
def _con_gemini(cx, usuario_id, pendiente, texto, cat):
    try:
        ...
        registro.aviso(f'  gemini no pudo: {ex}')
        return None
