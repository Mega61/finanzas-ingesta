...
from finanzas.adaptadores import firefly as F
from finanzas.adaptadores import graph

ENV_LOCAL = config.ruta_proyecto('.env')
# El cache del token lo define graph.py, que es su dueño. Aqui se lee, no se
# recalcula: tres copias de esta ruta ya se desincronizaron una vez.
CACHE_GRAPH = graph.CACHE
...
def ok(m):
    print(f'  {VERDE}OK{FIN}    {m}')


def mal(m):
    print(f'  {ROJO}FALLA{FIN} {m}')


def aviso(m):
    print(f'  {AMAR}OJO{FIN}   {m}')
...
from finanzas import config  # noqa: E402
...
def probar_firefly():
    ...
    try:
        ...
        activos = [
            n for n, d in cuentas.items() if d['type'] in ('asset', 'liabilities')
        ]
...
def probar_graph():
    ...
    autoridad = (
        f'https://login.microsoftonline.com/{E.get("GRAPH_AUTHORITY", "consumers")}'
    )
    ...
    if not res:
        ...
        if 'user_code' not in flujo:
            mal(
                f'no pude iniciar el device flow: {flujo.get("error_description", flujo)}'
            )
            nota("casi siempre es que falta poner 'Allow public client flows' = Yes")
            return False
        print()
        print(
            f'  {AMAR}==> Abre {flujo["verification_uri"]} y escribe el codigo:  '
    ...
    try:
        ...
        ok(
            f'cuenta: {yo.get("userPrincipalName") or yo.get("mail") or yo.get("displayName")}'
        )
    ...
    try:
        ...
        msgs = r.get('value', [])
        ok(
            f'lectura de correo funciona — {len(msgs)} alertas de Bancolombia en la muestra'
        )
        for m in msgs[:3]:
            nota(f'{m.get("receivedDateTime", "")[:10]}  {m.get("subject", "")[:58]}')
        if not msgs:
            aviso(
                'no encontro alertas. Revisa que este buzon sea el que recibe los correos'
                ' del banco, o que no esten en una carpeta excluida de la busqueda.'
            )
    except urllib.error.HTTPError as ex:
        mal(
            f'no pude leer correo: HTTP {ex.code} {ex.read().decode("utf-8", "replace")[:200]}'
        )
...
def probar_gmail():
    ...
    try:
        ...
        nota(
            'causas tipicas: falta activar verificacion en 2 pasos, es la clave normal'
        )
...
def probar_telegram():
    ...
    try:
        ...
        if chats:
            ...
            aviso(
                f'usuario @{b.get("username")}, mandale /start, y vuelve a correr esto.'
            )
    except Exception as ex:
...
def main(argv=None):
    ...
    if not os.path.exists(ENV_LOCAL):
        ...
        print(
            f'{GRIS}Copia .env.ejemplo a .env y llena lo que tengas. '
