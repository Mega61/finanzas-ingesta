...
```bash
cp .env.ejemplo .env
cp productos.ejemplo.csv productos.csv
...
python verificar.py graph      # solo uno
```
...
```bash
python -m demonio ciclo              # SECO: no escribe nada
python -m demonio ciclo --en-serio   # ahora sí
```
