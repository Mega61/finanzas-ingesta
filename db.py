...
def usuario_upsert(cx, nombre, firefly_url, firefly_token, telegram_chat_id=None):
    return Almacen(cx).guardar_usuario(nombre, firefly_url, firefly_token,
                                       telegram_chat_id)


def buzon_upsert(cx, usuario_id, proveedor, direccion, secreto=None,
                 imap_host=None):
    return Almacen(cx).guardar_buzon(usuario_id, proveedor, direccion, secreto,
                                     imap_host)
...
def correo_guardar(cx, buzon_id, message_id, remitente, asunto, fecha_correo,
                   cuerpo):
    return Almacen(cx).guardar_correo(buzon_id, message_id, remitente, asunto,
                                      fecha_correo, cuerpo)
...
def regla_guardar(cx, usuario_id, patron, cuenta_firefly=None, categoria=None,
    ...
    Almacen(cx).guardar_regla(usuario_id, patron, cuenta_firefly, categoria,
...
def regla_acierto(cx, regla_id):
    cx.execute('UPDATE reglas SET aciertos = aciertos + 1 WHERE id = ?',
               (regla_id,))
...
def bitacora(cx, accion, usuario_id=None, pendiente_id=None, firefly_id=None,
    ...
    Almacen(cx).anotar(accion, usuario_id, pendiente_id, firefly_id, payload,
                       respuesta, ok)
...
if __name__ == '__main__':
    ...
    _alm = Almacen(_cx)
    print(f'base: {ruta()}')
    for _t in ('usuarios', 'buzones', 'correos_crudos', 'pendientes', 'reglas',
               'bitacora', 'sugerencias', 'propuestas', 'preguntas_enviadas'):
        print(f'  {_t:20} {_alm.contar_por_tabla(_t):6}')
