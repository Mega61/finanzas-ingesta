...
def paso_asegurar_usuario(cx):
    ...
    if config.get('GMAIL_USUARIO') and config.get('GMAIL_APP_PASSWORD'):
        db.buzon_upsert(cx, uid, 'imap', config.get('GMAIL_USUARIO'),
...
def paso_importar(cx, uid, carpeta=None):
    ...
    for f in archivos:
        ...
        _, era_nuevo = db.correo_guardar(
            cx, b['id'], mid, str(msg.get('from') or ''), str(msg.get('subject') or ''),
            str(msg.get('date') or ''), cuerpo)
...
def paso_procesar(cx, uid):
    ...
    for c in filas:
        ...
        d = clasificador.clasificar(cx, c['usuario_id'], {
            'tipo': ev.tipo, 'fecha': ev.fecha, 'instrumento': ev.instrumento,
            'clase_instrumento': ev.clase_instrumento,
            'traslado_a': ev.traslado_a, 'contraparte': ev.contraparte,
        ...
        _, era_nuevo = db.pendiente_crear(
            cx, correo_id=c['id'], usuario_id=c['usuario_id'],
            tipo=ev.tipo, fecha=str(ev.fecha) if ev.fecha else None, hora=ev.hora,
            moneda=ev.moneda, valor=ev.valor, instrumento=ev.instrumento,
            clase_instrumento=ev.clase_instrumento, traslado_a=ev.traslado_a,
            contraparte=ev.contraparte, descripcion=ev.descripcion,
