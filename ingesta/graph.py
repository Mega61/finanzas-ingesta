...
def _app():
    ...
    autoridad = ('https://login.microsoftonline.com/'
                 + config.get('GRAPH_AUTHORITY', 'consumers'))
    cache = msal.SerializableTokenCache()
    if os.path.exists(CACHE):
        with open(CACHE, encoding='utf-8') as fh:
            cache.deserialize(fh.read())
    app = msal.PublicClientApplication(client_id, authority=autoridad,
                                       token_cache=cache)
    return app, cache
...
def cuerpo_plano(msg):
    ...
    if (b.get('contentType') or '').lower() == 'html':
        t = re.sub(r'<(script|style)[^>]*>.*?</\1>', ' ', contenido,
                   flags=re.S | re.I)
        t = re.sub(r'<[^>]+>', ' ', t)
        t = t.replace('&nbsp;', ' ').replace('&amp;', '&')
...
def bajar(cx, buzon_id, desde=None, tope=None, interactivo=False):
    ...
    for m in mensajes(tok, desde=desde, tope=tope):
        ...
        _, era_nuevo = db.correo_guardar(
            cx, buzon_id, mid, frm, m.get('subject'),
            m.get('receivedDateTime'), cuerpo_plano(m))
        if era_nuevo:
            nuevos += 1
        else:
            repetidos += 1
...
if __name__ == '__main__':
    tok = token(interactivo=True)
    print("token ok. ultimos 5 correos del banco:\n")
    for m in mensajes(tok, tope=5):
        cuerpo = cuerpo_plano(m)
        frase = re.sub(r'\s+', ' ', cuerpo)[:150]
